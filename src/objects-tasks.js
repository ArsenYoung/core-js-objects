...
function mergeObjects(objects) {
  ...
  const result = {};
  Object.assign(result, objects[0]);

  for (let i = 1; i < objects.length; i += 1) {
    const objProperties = Object.entries(objects[i]);
    for (let j = 0; j < objProperties.length; j += 1) {
      const [prop, value] = objProperties[j];
      if (prop in result) {
        result[prop] += value;
      } else {
        result[prop] = value;
...
function compareObjects(obj1, obj2) {
  ...
  const obj2properties = Object.entries(obj2);

  if (obj1properties.length !== obj2properties.length) {
    return false;
  }

  for (let i = 0; i < obj1properties.length; i += 1) {
    const [obj1key, obj1value] = obj1properties[i];
    const [obj2key, obj2value] = obj2properties[i];

    if (obj1key.localeCompare(obj2key) !== 0) {
      return false;
    }
...
function makeWord(lettersObject) {
  if (!lettersObject || typeof lettersObject !== 'object') {
    return null;
  }

  const wordObject = {};
  const lettersObjectEntries = Object.entries(lettersObject);
  for (let i = 0; i < lettersObjectEntries.length; i += 1) {
    const [letter, keys] = lettersObjectEntries[i];
    for (let j = 0; j < keys.length; j += 1) {
      const key = keys[j];
      wordObject[key] = letter;
    }
  }
  const wordObjectValues = Object.values(wordObject);

  return wordObjectValues.join('');
}
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    if (queue[i] !== seller.tiketPrice) {
      const change = queue[i] - seller.tiketPrice;
      if (change > seller.change) {
        return false;
      }

      seller.change -= change;
    }
    seller.change += queue[i];
  }
...
function Rectangle(width, height) {
  const rectangle = new Rect(width, height);
  return rectangle;
}
